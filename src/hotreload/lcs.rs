...
pub fn diff_sequences(old: &[StableId], new: &[StableId]) -> LcsResult {
    ...
    // Build index maps
    let old_map: HashMap<StableId, usize> = old.iter().copied().enumerate().map(|(i, id)| (id, i)).collect();
    let new_map: HashMap<StableId, usize> = new.iter().copied().enumerate().map(|(i, id)| (id, i)).collect();
